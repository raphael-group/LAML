...
class PMMNTest_scipy_opt(VirtualUnitTest):
    ...
    def test_1(self):
        ...
        for T,true_nllh in zip(tree_list,true_nllh_list):
            ...
            self.assertAlmostEqual(true_nllh,test_nllh,places=4,msg="PMMNTest Scipy-opt: test_1 failed.")
    ...
    def test_2(self):
        ...
        for T,true_nllh in zip(tree_list,true_nllh_list):
            myModel = PMMN_model([T],{'DLT_data':DLT_data},{'Q':Q},{'mu':1,'nu':0,'phi':0})
            test_nllh,_ = myModel.optimize('Scipy',initials=1,verbose=-1,ultra_constr=False,fixed_params={'rho':1})
            self.assertAlmostEqual(true_nllh,test_nllh,places=4,msg="PMMNTest Scipy-opt: test_2 failed.")
