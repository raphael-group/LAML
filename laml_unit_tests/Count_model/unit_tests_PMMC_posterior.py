...
class PMMCTest_posterior(VirtualUnitTest):
    # test posterior computation
    def check_posterior(self,alleleTable,Q,T,phi,nu,rho,test_no): ##### only works if K = 1, J = 1, and alphabet = {0,1,-1} #####
        ref_file = pkg_resources.resource_filename('laml_unit_tests', 'test_data/test_Count_model/test_PMMC/test_posterior/test_'+str(test_no)+'.txt')
        ...
        myModel = PMMC_model([T],{'DLT_data':DLT_data},{'Q':Q},{'mu':1,'nu':nu,'phi':phi,'rho':rho})
        myModel.Estep()
        ...
        for node in myModel.trees[0].traverse_postorder():
            ...
            if node.label in ref_values['post_node_z']:
                ...
                self.assertAlmostEqual(true,est,places=5,msg="PMMCTest posterior: test_" + str(test_no) +  " failed for post_node_z.")
            # test post_node_s
            if node.label in ref_values['post_node_s']:
                ...
                self.assertAlmostEqual(true,est,places=5,msg="PMMCTest posterior: test_" + str(test_no) +  " failed for post_node_s.")
            # test post_edge_zz
            if node.label in ref_values['post_edge_zz']:
                true = ref_values['post_edge_zz'][node.label]
                est = node.log_edge_posterior[0][((0,),(0,))]
                self.assertAlmostEqual(true,est,places=5,msg="PMMCTest posterior: test_" + str(test_no) + " failed for post_edge_zz.")
            # test post_edge_za
            if node.label in ref_values['post_edge_za']:
                true = ref_values['post_edge_za'][node.label]
                est = node.log_edge_posterior[0][((0,),(1,))]
                self.assertAlmostEqual(true,est,places=5,msg="PMMCTest posterior: test_" + str(test_no) + " failed for post_edge_za.")
            # test post_edge_zs
            if node.label in ref_values['post_edge_zs']:
                true = ref_values['post_edge_zs'][node.label]
                est = node.log_edge_posterior[0][((0,),(-1,))]
                self.assertAlmostEqual(true,est,places=5,msg="PMMCTest posterior: test_" + str(test_no) + " failed for post_edge_zs.")
            # test post_edge_aa
            if node.label in ref_values['post_edge_aa']:
                true = ref_values['post_edge_aa'][node.label]
                est = node.log_edge_posterior[0][((1,),(1,))]
                self.assertAlmostEqual(true,est,places=5,msg="PMMCTest posterior: test_" + str(test_no) + " failed for post_edge_aa.")
            # test post_edge_as
            if node.label in ref_values['post_edge_as']:
                true = ref_values['post_edge_as'][node.label]
                est = node.log_edge_posterior[0][((1,),(-1,))]
                self.assertAlmostEqual(true,est,places=5,msg="PMMCTest posterior: test_" + str(test_no) + " failed for post_edge_as.")

    def test_1(self): 
        Q = [[{1:1}]]
        alleleTable = {'a':[{1:1}],'b':[{1:1}],'c':[{1:1}]}
        T = "(((a:1,b:1)ab:1,c:1)abc:1)r;"
        phi = 0
        nu = 0
        rho = 1
        test_no = 1
        self.check_posterior(alleleTable,Q,T,phi,nu,rho,test_no)
    
    def test_2(self): 
        Q = [[{1:1}]]
        alleleTable = {'a':[{}],'b':[{}],'c':[{1:1}],'d':[{0:1}]}
