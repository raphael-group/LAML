...
class Mstep_PMM_base:
    def __init__(self,model_obj,ultra_constr_cache=None,local_brlen_opt=True,nIters=1):
        ...
        self.N = N
        self.N_free = N_free
        self.C_z2z = np.zeros(N)
        self.C_z2a = np.zeros(N)
        self.C_a2a = np.zeros(N)
        self.C_za2s = np.zeros(N)

        self.d_ini = np.zeros(N)
        self._j2i = np.zeros(N_free,dtype='int')
        i = 0
        j = 0
        for tree in model_obj.trees:
...
class Mstep_PMMsep(Mstep_PMM_base):
    def optimize_brlen(self,nu,verbose=False): # nu is a single number
        ...
        SC = -nu*self.C_a2a[self._j2i].T @ var_d
        SD = (self.C_za2s[self._j2i]).T @ cp.log(1-cp.exp(-nu*var_d)) if sum(self.C_za2s[self._j2i]) > 0 and nu > eps_nu else 0
